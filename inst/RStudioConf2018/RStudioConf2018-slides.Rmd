...
- [1] accepts the user submission from `learnr`. This is always called `USER_CODE`. The function `for_checkr()` does some pre-processing of the user submission to turn it into evaluated code and format it for use in later `checkr` functions.
...
res <- pre_check(USER_CODE) # putting this here anticipating integrating for_check with pre_check
if (failed(res)) return(res)

hello_fun_strict(USER_CODE)
```

```{r hello-world-strict-code-check}
3
```
